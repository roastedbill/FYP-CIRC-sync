...
do
	...
	if [ "$k" -gt "1" ]; then
		...
		/apps/sysapps/TORQUE/bin/qsub -q circ-spool -v output_dir=${output_dir},input_dir=${input_dir},name=${patient_name},i=${count} /share/users/imganalysis/yeolab/data/ADNI/ADOnly/preprocessed/base/scripts/submit_job_base.sh
	fi
done
echo $count
